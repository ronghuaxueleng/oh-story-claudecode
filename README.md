...
网文写作 skill 包，覆盖长篇与短篇网络小说的扫榜、拆文、写作、去AI味、封面图全流程。当前维护线为 Codex 专用分支。
...
使用 `/story-short-analyze` 拆解短篇《曾将爱意私藏》（约 8500 字，追妻火葬场 · 死遁）的新版完整输出：
...
短篇拆文现在不再只产出 `拆文报告 / 情节节点 / 写作手法` 三件套。
如果后续目标是 `仿写 / 融合 / 去原作化后再写`，默认还要继续落盘：

- 16 张 `可直接仿写_*.md`
- `原文细节库/`
- `写作资产/`

下游 `/story-short-write` 会优先读取这些资产，而不是只读一份长报告。
