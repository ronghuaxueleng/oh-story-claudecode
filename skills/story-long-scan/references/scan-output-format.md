...
排名 | 书名(详情页优先) | 作者(详情页优先) | 状态 | 在读(核心指标) | 字数(万字) | 分类(主分类) | 最新更新 | 作品页链接 | 简介(截断100字)
...
推荐优先用 `scripts/fanqie-rank-auto.js`：
- `--mode auto`：先试 HTTP，失败再回退到 browser-cdp
- `--mode http`：强制纯后台 HTTP
- `--mode cdp`：强制 browser-cdp

HTTP 路径底层使用 `scripts/fanqie-rank-http-scraper.js`：
- 直接请求榜单页，读取 `window.__INITIAL_STATE__`
- 逐本请求 `/page/{bookId}`，以详情页里的真实 `bookName` / `authorName` / `abstract` 为准
- 纯后台运行，不打开浏览器窗口

HTTP 方案注意事项：
- 详情页 `window.__INITIAL_STATE__` 偶发不是严格 JSON，可能含 `undefined`，需要用 JS 对象字面量容错解析
- 详情页偶发返回空壳页或缺少 `__INITIAL_STATE__`，必须重试，且允许单条降级
- `completeCategory` 可能是 JSON 字符串数组，要提取 `MainCategory=true` 的 `Name` 作为主分类
- 榜单页 `bookName` / `author` 可能仍带字体混淆，只能作为降级兜底字段

browser-cdp 仍可作为补充：
- 用 `scripts/fanqie-rank-scraper.js` 通过详情页绕过字体反爬
- 单页上限约20本，需滚动加载
- 流程：访问品类页→提取品类链接→逐品类采集
...
| 污染类型 | 处理 |
|----------|------|
| 详情页空壳页（无 `__INITIAL_STATE__`） | 重试 2-3 次；仍失败则单条降级 |
| 状态对象含 `undefined` / 非严格 JSON | 改用对象字面量解析，不直接 `JSON.parse` |
| 分类字段是 JSON 字符串 | 提取主分类名称，不整串落盘 |
| 榜单页字体混淆标题/作者 | 用详情页真实字段覆盖 |
| 详情页作者简介、平台模板文案混入摘要 | 删除模板文案，仅保留作品简介 |
