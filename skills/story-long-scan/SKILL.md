...
| 优先级 | 模式 | 说明 | 何时用 |
|--------|------|------|--------|
| 1 | **脚本采集** | 直接抓取平台页面或 SSR 数据，后台产出结构化文件 | 优先；起点默认不需要 Chrome |
| 2 | **browser-cdp 采集** | 直接抓取平台页面，产出结构化文件 | 需要登录态、需要执行前端脚本、脚本直抓失效时 |
| 3 | **用户提供** | 用户粘贴榜单截图/文字/链接 | 用户已有数据时 |
| 4 | **内置知识** | 基于知识库趋势数据做分析 | 无法联网、用户无数据时 |

#### 脚本采集模式

优先用于有稳定脚本方案的平台。当前最适合：
- 起点：优先移动端 SSR，再回退到 CDP/PC
- 番茄：优先后台 HTTP/SSR，失败再回退到 browser-cdp
...
**起点推荐采集顺序**：
1. 默认运行 `scripts/qidian-rank-scraper.js --mode auto`
   - 先读 `https://m.qidian.com` 移动端 SSR pageContext
   - 若移动端失败，再回退到 CDP/PC 页面
2. 若明确要只走移动端，用 `--mode mobile`
3. 若明确要只走浏览器态，用 `--mode cdp`

**起点补充说明**：
- `auto` 模式优先用移动端 SSR，能绕开 PC 站部分风控页
- 回退到 CDP 后，脚本会检测验证码/安全验证，并做有限重试
- 自动重试仍失败时，允许用户在浏览器里手动过验证，脚本轮询等待后继续
...
- Phase 4 不只口头给方向，必须按 [references/topic-decision.md](references/topic-decision.md) 产出一份 `选题决策.md`
- 若本轮已有明确输出目录，落盘到该目录
- 若本轮是给现成小说项目做前置扫榜，允许直接落为项目内的 `设定/选题决策.md`
- `选题决策` 只负责“方向决策”，不得冒充 `开书总蓝图`
...
| [references/topic-decision.md](references/topic-decision.md) | 需要把扫榜结果收束成可开书方向与验证动作时 |
| [references/publishing-guide.md](references/publishing-guide.md) | 平台选择+推荐机制+数据指标+简介设计 |
| [references/scan-output-format.md](references/scan-output-format.md) | 脚本采集 + browser-cdp 采集字段定义+输出模板+文件命名规范 |
...
| [scripts/qidian-rank-scraper.js](scripts/qidian-rank-scraper.js) | 起点榜单采集（畅销/月票/新书等），默认移动端 SSR 提取，失败再回退到 CDP/PC |
