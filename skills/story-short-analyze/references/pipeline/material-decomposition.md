...
情绪走法不能只落在 `情绪母线.md` 的总结里。每个进入 `可直接仿写_顺序事件表.md` 的承重节点，还必须同步记录：

1. `读者情绪拍`：进入压力、受辱/刺痛、短暂希望或反抗、反刀、场末余痛中的哪一拍。
2. `情绪烈度`：1-10 分，只用于同一本书内部比较。
3. `是否反刀或峰值`：明确本节点是不是反刀拍、峰值拍，不能靠后续写作者猜。
4. `场末余痛`：这一场结束后读者带着什么未消退的感觉进入下一场。

这些字段是后续仿写的直接资产。仿写不是只搬“发生了什么”，而是要让新稿在同一位置给读者同级感受；如果拆书没有留下逐拍证据，写作侧就无法证明情绪同构。
...
那在收口前必须先补 10 张基础表，再补 3 份写作资产硬门槛：
...
11. `作者DNA指纹`
   - 这个作者最稳定怎么切句、怎么停、怎么把情绪落到动作和物件上
   - 哪些旧伤触发器、惯用落点会反复出现
12. `仿写约束_禁写清单`
   - 哪些总结句、过渡句、解释句、假口语、补字方式一写就会假
   - 哪些“看起来很会写”的句子其实最容易把稿子写熟
13. `同桥段过检规则`
   - 相同桥段里原文为什么能过，仿稿最容易死在哪
   - 这个桥段的承重件、脏感来源、动作先后、口气毛边到底是什么

只要基础 10 张表没出来，说明拆文还停在概括层；只要这 3 份写作资产没出来，说明还没拆到“可直接安全仿写”。
