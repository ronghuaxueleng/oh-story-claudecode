...
`## 13. 场景因果资产` 每张卡固定以 `- 因果资产：CPA-xx 名称` 起卡，嵌套写 `到场原因 / 知情边界 / 物件生命周期 / 制度约束 / 明显替代方案阻断 / 离场因果 / 原文证据`。这些内容必须由 `book.profile.json.causal_precondition_assets` 原样结构化承接，禁止下游临写细纲时再猜。
