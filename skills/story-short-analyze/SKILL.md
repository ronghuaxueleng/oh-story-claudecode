...
- 每个 `子流程索引.jsonl` 的 SF 必须在 finalize 前固化逐场 `source_style_granularity`：叙述者态度、句间关系与节奏、段落气口与切点、对白错答/回避、动作感知情绪织法、即时插嘴与粗粝度六项缺一不可；每项至少两条证据且必须位于该 SF 的 `source_range` 内，六项合计至少覆盖四条不同证据，同一证据组不得覆盖四个及以上字段。不得用全书通用文风摘要、同一五条模板或写作阶段人工补填代替。
