...
9. 每个 BID 必须继续下钻为一个或多个 `SF-*` 完整子流程。子流程不是动作零件，而是可独立迁移的连续流程，必须保留进场状态、完整动作/反应顺序、信息延迟、控制权变化和场末状态；`子流程索引.jsonl` 每条必须能回指 `子流程施工卡.md` 与原文。
10. `profile_source.md` 同时服务结构化抽取和单书厚规则包；`桥段施工卡.md` 继续承担更厚的人类施工解释层，但不能把桥规则骨架全甩给施工卡
11. `拆文报告.md`、`写作手法.md`、`写作资产/样本分级与可学层.md` 必须共同承接全局成文形状审计；只在细节表或模型备注中提及不算完成
12. `book.profile.json` 由脚本生成，不与 Markdown 同批手写
13. 收口必须跑：
