...
- `agents_version: 21` → 旧版，只有 SF 级六维全集，缺独立逐层来源拓扑与逐层验收
- `agents_version: 22` → 当前版本
...
- 短篇拆文同时部署 `子流程索引.jsonl` 的 SF 整链合同与 `子流程层次索引.jsonl` 的规范化逐层来源合同，并由 `validate_subflow_catalog.py` 合并校验逐字原文、层序和全文覆盖
- 短篇写作合同升级到 v7：每个来源层必须绑定目标细拍、保留原层型并填写本层专属换壳说明，不能再以 SF 级六条摘要代替
- 初稿终审升级到 v5：逐层核验层型、连接、叙述距离与六维协同；判刑急刹、舆论粗跳和传闻冷尾不会被错误扩成统一调查说明
- 已部署项目需重新运行 `story-setup`，让版本标记升级到 `agents_version: 22`
