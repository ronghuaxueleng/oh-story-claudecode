...
- `hot_news_materials`：默认空数组；仅用户明确要求热点时记录来源、可迁移机制和事实边界。
...
- 未明确启用热点时，`hot_news_materials` 与全部 `news_ids` 必须为空。启用后至少两条不同新闻落到两个目标 P 拍；只有一个主体 P 拍时最低数量降为一，新闻发布至检索不得超过 90 天。
