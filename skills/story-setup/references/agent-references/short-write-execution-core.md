...
2. **热点换芯与细纲**：保留主体完整上层层级和 E 拍，检索热点现实机制，逐拍重建目标 P 拍，再完成 `小节大纲.md`。
3. **迁移合同**：映射主体 P 槽位、主体 E 和辅助选中 P 到目标细拍 ID，逐拍填写 P 拍换芯判断与热点来源；主体 SF 六维按原文行区间和 P 映射自动派生。
4. **正文**：放行后按区域同时展开 BID/E、目标 P、热点机制和主体 SF 六维，直接顺序写 `正文.md`，不创建逐节行政回执。
5. **合并终审**：一次检查每个区域的 P 换芯、E、场面、SF 六维、热点落地和声线及全局层级、题面、事实边界、开头、结尾、长句和对白，然后进入初稿停靠。
...
| `细纲表演验收回执.json` | 完整主体层级、三组同序映射、逐拍 P 换芯、热点来源，以及确定性派生的主体 SF 六维区域覆盖 |
| `正文.md` | 唯一初稿正文 |
...
通过后直接写正文。主体原文、主体 profile 和区域对应 SF 六维负责声线，细纲合同负责上层结构、E 拍顺序和目标 P 拍，热点与辅助来源只提供已选机制。
