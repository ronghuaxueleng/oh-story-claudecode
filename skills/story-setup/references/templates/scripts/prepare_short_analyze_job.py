...
def write_required_manifest(path: Path, book_name: str, source_path: Path, layout: ContractLayout) -> None:
    payload = {
        ...
        "final_gate": {
            "prepare": f"python3 scripts/prepare_short_analyze_job.py \"{source_path}\"",
            "finalize": f"python3 scripts/run_short_analyze_finalize.py \"拆文库/{book_name}\" --json",
        },
    }
    dump_json(path, payload)
...
def write_progress(path: Path, book_name: str, layout: ContractLayout) -> None:
    ...
    lines.extend([
        "",
        "## 最终验收",
        f"- [ ] `python3 scripts/run_short_analyze_finalize.py \"拆文库/{book_name}\" --json`",
        "",
    ])
...
def write_execution_prompt(path: Path, book_name: str, source_copy: Path, out_dir: Path) -> None:
    lines = [
        ...
        "",
        "## 固定执行顺序",
        "1. 先读项目内的短篇拆书 skill 说明或当前执行说明",
        "2. 再按固定顺序产出全套文件，不允许只写主报告",
        "3. 按 `主报告批 -> 16张表批 -> 原文细节库批 -> 写作资产批 -> profile批 -> 验收批` 完整落盘",
        "4. 最后运行：",
        f"   `python3 scripts/run_short_analyze_finalize.py \"{out_dir}\" --json`",
        "",
        "## 强约束",
        "- 不能只产三件套后停下",
        "- 不能把 16 张表写成同一种说明腔",
...
def prepare(args: argparse.Namespace) -> dict:
    ...
    created = {
        ...
        "next_step": f"python3 scripts/run_short_analyze_finalize.py \"{out_dir}\" --json",
    }
    return created
