...
def write_execution_prompt(
    ...
    lines = [
        ...
        "- 事实台账后在同一次逐行扫描中建立 `写作资产/全文情绪颗粒总账.json`、`写作资产/全文情节微拍总账.json`、SF 整链 `写作资产/子流程索引.jsonl` 与规范化逐层目录 `写作资产/子流程层次索引.jsonl`；E/P 不共用 ID，SF 整链与逐层精确原文、层间关系及六维协同也不得互相代替；BID 只能在两份总账完成后归纳",
