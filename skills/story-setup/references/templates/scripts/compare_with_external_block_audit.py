...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="对齐外部分块审计 PDF 与 full audit JSON，生成代理校准摘要。")
    parser.add_argument("root", help="包含 txt/md 与外部分块审计 pdf 的目录")
    parser.add_argument("--audit-dir", required=True, help="run_full_ai_audit.py 输出目录")
    parser.add_argument("--output", default="external_block_audit_alignment.csv", help="输出 CSV 路径")
    parser.add_argument(
        "--summary-output",
        default="external_block_audit_alignment_summary.json",
        help="输出摘要 JSON 路径",
    )
    parser.add_argument(
...
def parse_external_block_audit_pdf(pdf_path: Path) -> dict:
...
def build_rows(root: Path, audit_dir: Path, strict: bool = False) -> tuple[list[dict], list[dict]]:
    ...
    for pdf_path in sorted(root.rglob("*.pdf")):
        ...
        if not source_path:
            continue
        try:
            audit_report = parse_external_block_audit_pdf(pdf_path)
        ...
        row = {
            ...
            "external_block_audit_weighted_avg": audit_report["weighted_avg"],
            "external_block_audit_max_seg": audit_report["max_seg"],
            "external_block_audit_min_seg": audit_report["min_seg"],
            "external_block_audit_segment_count": audit_report["segment_count"],
        }
        if audit:
            row.update(
                {
...
def correlation_rows(rows: list[dict]) -> list[dict]:
    ...
    for metric in metrics:
        ...
        out.append(
            {
                "metric": metric,
                "vs_external_block_audit_weighted_avg": spearman(xs, [float(row["external_block_audit_weighted_avg"]) for row in metric_rows]),
                "vs_external_block_audit_max_seg": spearman(xs, [float(row["external_block_audit_max_seg"]) for row in metric_rows]),
            }
        )
    return out
...
def pick_best_metric(correlations: list[dict], target_key: str, positive_only: bool = False) -> dict | None:
    ...
    ranked.sort(
        key=lambda item: (
            ...
            abs(parse_float(item.get("vs_external_block_audit_weighted_avg")) or 0.0),
        ),
        reverse=True,
    )
    return ranked[0]
...
def build_summary(rows: list[dict], failures: list[dict], correlations: list[dict], args: argparse.Namespace) -> dict:
    best_weighted = pick_best_metric(correlations, "vs_external_block_audit_weighted_avg", positive_only=True)
    best_max_seg = pick_best_metric(correlations, "vs_external_block_audit_max_seg", positive_only=True)
    fallback_weighted = pick_best_metric(correlations, "vs_external_block_audit_weighted_avg")
    fallback_max_seg = pick_best_metric(correlations, "vs_external_block_audit_max_seg")
    segment_counts = [int(row["external_block_audit_segment_count"]) for row in rows if row.get("external_block_audit_segment_count") is not None]
    return {
        ...
        "external_block_audit_segment_count_range": {
        ...
        "best_metric_vs_external_block_audit_weighted_avg": best_weighted,
        "best_metric_vs_external_block_audit_max_seg": best_max_seg,
        "fallback_metric_vs_external_block_audit_weighted_avg": fallback_weighted,
        "fallback_metric_vs_external_block_audit_max_seg": fallback_max_seg,
        "calibration_models": {
            "external_block_audit_weighted_avg": build_best_multi_calibration(rows, "external_block_audit_weighted_avg"),
            "external_block_audit_max_seg": build_best_multi_calibration(rows, "external_block_audit_max_seg"),
        },
        "recommendation": {
            ...
            "note": "优先盯最高外部块的正相关代理值，因此先看 vs_external_block_audit_max_seg 的正相关指标；全文平均只作辅助。",
        },
    }
...
def build_internal_standard(summary: dict) -> dict:
    return {
        ...
        "calibrated_from": "external_block_audit_pdf_alignment",
        "sample_count": summary.get("sample_count"),
        "parse_failure_count": summary.get("parse_failure_count"),
        "segment_count_range": summary.get("external_block_audit_segment_count_range"),
        ...
        "legacy_source": {
            "best_metric_vs_external_block_audit_weighted_avg": summary.get("best_metric_vs_external_block_audit_weighted_avg"),
            "best_metric_vs_external_block_audit_max_seg": summary.get("best_metric_vs_external_block_audit_max_seg"),
            "fallback_metric_vs_external_block_audit_weighted_avg": summary.get("fallback_metric_vs_external_block_audit_weighted_avg"),
            "fallback_metric_vs_external_block_audit_max_seg": summary.get("fallback_metric_vs_external_block_audit_max_seg"),
        },
        "notes": [
            "这是一份内部标准文件。校准来源是历史外部分块审计样本，但后续日常使用只依赖本文件。",
