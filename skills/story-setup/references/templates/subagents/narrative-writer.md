...
2. **展开子事件**：每个子事件将发生、感知、反应三维度织入同一段连续正文（合计 ≥100-150 字）
   ...
   - **手机阅读密度**：常规叙事优先一段一句；细密描写最多 2 句；单段 >60 字或单句 >35-45 字时，优先拆短
   - **输出前密度重排**：扫描每段；>60 字先按句号拆段，单句仍 >45 字则按逗号/动作转折拆句；一段里不要连续放 3 个句号
3. **收尾**：钩子或情绪定格（1-2 句）
...
- **Gate D 节奏打碎**：长句拆短、同构句打散（核心规则：优先一段一句，细密描写最多 2 句，单段 <= 60 字；输出前按句号/动作转折做密度重排）
...
- **字数不足时的强制扩充流程**（按优先级依次尝试，直到达标）：
  1. **回到细纲补充子事件**（最高优先级）：检查细纲是否有遗漏的情节点，补充新的子事件/冲突/转折，然后用三维度织入将这些新子事件写成正文
  2. 增加子事件（用三维度织入展开已有的、展开不够的子事件）
  3. 加一轮对话（参考 writing-craft.md 对话权力模式，推动关系/冲突）
  4. 加回忆闪回（1-2 句关联记忆，必须与当前情境有因果关联）
  5. 织入感官细节（感官细节织入动作节拍中，不单独成句）
  6. 织入身体反应（身体动作织入事件中，不单独成段）
...
- 格式合规：一段一句优先、细密描写最多 2 句、<=60字、无空行、对话独立成行、无「他说/她道」
- 节奏均匀度：是否有连续多节无情绪变化？
- 身体部位重复：同一词全文 <= 5 次
- 「像」使用频率：全文不超 10 处
...
- **禁止信息过载**：三维度织入后一段到底；一段超过 60 字、一段超过 2 句、或一句超过 35-45 字 -- 必须按新动作/新物件/新信息/新对话拆分
