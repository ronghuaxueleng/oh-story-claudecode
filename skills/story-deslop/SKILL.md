...
description: |
  ...
  触发方式：提到 `story-deslop`、`去AI味`，或直接说「去AI味」「去味」「deslop」「这篇太AI了」
---
...
- 删除比例上限：单次去AI味操作的删除量不得超过原文总字数的 15%（优先用 Python 字符统计修改前后字符数差值，`wc -m` 仅作 Unix 备选）。超过此比例应提醒用户并建议分段处理
...
Phase 2 诊断完成后，如果项目已部署 `narrative-writer`（**必须先检查 `.codex/agents/narrative-writer.md` 是否存在**），可 spawn 一个子代理：
...
模式处理：按 references/anti-ai-writing.md 的问题模式目录执行；所有新增模式都归入 Gate A-F 的对应处理。相邻段重复表达同一信息/动作/情绪时，按 Gate C/D 合并去重；字数不足补新子事件或对话。
```

执行去AI味操作（6 Gate + 三遍法）。如 `narrative-writer` 未部署，由主线程直接执行。
...
- 若合并后节奏过快，补充新的子事件、对话或冲突，不在原动作后追加描写层
