...
from search_plot_library import (  # noqa: E402
    ...
    _passes_interlude_length,
    _safe_chapter_no,
    _score_interlude_plot,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="从 Neo4j 或本地 JSON 情节库中搜索可改编的微型子任务")
    ...
    parser.add_argument("--length", choices=["light", "short"], default="light", help="子任务长度：light=单章，short=2-3章")
    ...
    if args.json_library:
        ...
        for plot in plots:
            ...
            candidates.append({
                ...
                "micro_task": _build_micro_task(plot, structure_terms=args.structure_terms),
            })
    else:
    ...
    candidates.sort(
        key=lambda x: (
            -x["score"],
            str(x["novel_id"]),
            _safe_chapter_no(x["micro_task"].get("start_chapter")),
        )
    )
    result = {
