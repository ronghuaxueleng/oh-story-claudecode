...
优先使用 `chapter-extractor` 子代理并行处理。每章 spawn 一个子代理，每次 5-8 个并发。
子代理不可用时退回主线程串行处理。

子代理输出格式严格对齐本阶段 A/B/C 三部分的合并输出，详见 `output-templates.md` 的 Stage 2 模板。
...
Stage 2 使用 `chapter-extractor` 子代理并行处理（每章一个子代理，每批 5-8 个），不分块。
...
输入分块大小 6-8K token/块（中小型），章节边界对齐。输出长度限制见 SKILL.md（Stage 2 子代理模式按密度公式输出，Stage 3-5 单阶段 ≤8000 中文字符）。每块完成后更新 `_progress.md`。
