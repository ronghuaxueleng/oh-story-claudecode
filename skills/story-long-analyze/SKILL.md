...
description: |
  ...
  单一深度拆解管道：跑完黄金三章后先产出快速预览，可停靠，也可继续全量拆解。
  触发方式：提到 `story-long-analyze`、`长篇拆文`，或直接说「帮我拆这本书」「分析黄金三章」
  「深度拆解」「完整拆解」「系统拆解」或提供小说文本文件路径时，进入全量管道
---
...
| 1 | 黄金三章 | 前3章原文 | 第1-3章_深度拆解.md + 快速预览.md | 3章拆解完成 |
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色）。角色过滤（龙套不提取、别名归类）。每章3-40情节点（密度150-200字/个，按字数动态调节）。**并行模式：每章 spawn `chapter-extractor` 子代理**。**计数验证：摘要数 == 章节数，不等则标记失败章节**。 | 所有章节处理完成 |
...
Stage 0-1 完成后，默认先生成 `拆文库/{书名}/快速预览.md`，给用户一个黄金三章 + 概要的早期判断快照。

- 如果用户一开始明确说的是「完整拆解 / 一次跑完 / 系统拆解 / 别停」，则保留 `快速预览.md`，但不停下询问，直接继续 Stage 2-5。
- 如果用户只是想先看黄金三章或快速判断，则在 Stage 1 停靠，等用户确认后再从 Stage 2 续跑。
- 续跑时不得重跑 Stage 0-1，必须读取 `_progress.md` 并从 Stage 2 开始。
...
则拆文阶段不得只输出“这本书怎么写”，还必须额外输出一层 `参考源分层结论`，供后续 `story-long-write` 直接承接。
...
- `只能转译后再借的层`
  ...
  - 高位站队方式
- `TXT 与情节库分工结论`
  - `TXT正文` 负责：起爆速度 / 压场方式 / 对白咬人程度 / 群像反应密度 / 章尾停拍
  - `情节库` 负责：发动机 / 收益档位 / 反压链 / 第三方迟疑成本 / 章尾新后果

执行要求：
- 拆文报告里不得只写“适合模仿文风”或“适合套情节库”，必须写清“到底借哪层，不借哪层”
- 若拆解结果准备供写作冷启动使用，必须显式写出：
  - `哪些层可直接进入参考输入卡`
  - `哪些层只能在写作时作为表演校准`
  - `哪些层一旦照搬就会同构`
- 若拆文对象已有情节库分层资料，不得把原始 TXT 的连续事件顺序直接上升为“可借主骨架”；必须先拆回发动机层
- 若拆文报告只复述“原书这章发生了什么”，而没有抽出“这一章为什么能挂人/为什么能改盘/为什么能逼第三方改线”，视为拆文不达标

建议输出位置：
- 快速模式：并入 `Phase 4 拆文报告`
- 深度模式：单列 `拆文库/{书名}/参考源分层结论.md`
...
Stage 2 使用 `chapter-extractor` 子代理并行处理每章，替代原来的串行分块。

### 调用方式

```text
spawn 子代理：chapter-extractor
输入：
章节编号：第{N}章
章节标题：{标题}
章节字数：{字数}
...
- 每次 spawn 5-8 个子代理（避免并发限制）
- 等待当前批次全部完成后，再 spawn 下一批
- 每批完成后更新 `_progress.md` 记录已处理章节

### 子代理输出收集

- 每个子代理返回 markdown 格式的提取结果
- 主线程将子代理输出写入 `章节/第{N}章_摘要.md`
- 收集所有子代理的出场人物表，供 Stage 3 合并使用
...
如果 `.codex/agents/chapter-extractor.md` 不存在（未部署），Stage 2 退回串行分块模式，由主线程按原分块策略处理。

---

## 分块策略

Stage 2 使用 `chapter-extractor` 子代理按章节并行，不分块。
...
- **输出长度上限**：Stage 2 `chapter-extractor` 子代理模式按密度公式输出（每章情节点总量由字数动态决定，子代理内部自控）。Stage 3-5 串行分块模式下，单阶段输出不超过 8000 中文字符（用 `wc -m` 统计，超出时优先保留情节点和角色数据，缩写环境描写和心理分析）
...
完整模板与状态说明见 [pipeline-ops.md](references/pipeline-ops.md)。
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 准备开写 | story-long-write | 使用 `story-long-write` |
| 需要市场数据 | story-long-scan | 使用 `story-long-scan` |
| 更适合短篇 | story-short-scan → story-short-analyze | 使用 `story-short-scan` |
...
| [references/pipeline-ops.md](references/pipeline-ops.md) | 需要停靠、恢复、查看 `_progress.md` 状态值时 |
