---
name: story
description: |
  网络小说工具箱主入口。根据用户需求自动路由到对应 skill。
  触发方式：提到 `story`、`网文`，或直接说「我想写小说」「帮我写书」「写网文」
  当用户意图不明确时触发此 skill，由路由逻辑分发到具体的扫榜/拆文/写作/去AI味/封面 skill。
---

# story：网文工具箱路由

你是网文工具箱的路由入口。用户的请求模糊时由你分发到具体 skill。

## 路由表

| 用户意图 | 关键词示例 | 路由到 |
|---|---|---|
| 写长篇 | 开书、写大纲、长篇、连载 | `story-long-write` |
| 写短篇 | 短篇、盐言、一万字 | `story-short-write` |
| 长篇拆文 | 拆文、分析这本书、黄金三章 | `story-long-analyze` |
| 短篇拆文 | 拆短篇、分析这个故事 | `story-short-analyze` |
| 长篇扫榜 | 长篇排行、什么火、起点/番茄/晋江 | `story-long-scan` |
| 短篇扫榜 | 短篇排行、知乎盐言排行 | `story-short-scan` |
| 去 AI 味 | 去 AI 味、太 AI、去味 | `story-deslop` |
| 封面 | 封面、封面图 | `story-cover` |
| 环境部署 | 准备写书、搭环境、初始化 | `story-setup` |
| 浏览器操控 | 浏览器、抓取、登录态 | `browser-cdp` |
| 导入小说 | 导入、反向解析、导入小说、把我的书导进来 | `story-import` |
| 查故事资料 | 查角色、查伏笔、查进度、查设定、什么状态、写到哪了 | 直接 spawn `story-explorer` 子代理（使用结构化 prompt：`项目目录：{dir}\n查询类型：{根据意图选择}\n查询参数：{用户查询}`） |
| 查资料 | 查资料、帮我查资料、调研、搜索一下、搜一下 | 直接 spawn `story-researcher` 子代理 |

## 路由流程

1. 分析用户请求，提取意图关键词
2. 匹配上表，找到对应的 skill
3. 如果能明确匹配，直接调用对应 skill（`Skill("skill-name")`）
4. 如果无法匹配，询问用户想做什么（从上表中选择）
5. 如果用户说"我想写小说"但未指定长篇/短篇，询问篇幅类型后再路由

## 冷启动测试补充规则

- 当用户明确提到：
  - `冷启动测试`
  - `重新冷启动`
  - `新开一个测试小说`
  - `每次只写一个文件`
- 则路由到对应写作 skill 时，必须一并附带以下执行提醒：
  - 当前轮次按 `单文件推进` 执行
  - 一次只允许新增或改写 `1` 个项目文件
  - 不得在同一轮里顺手补齐同目录下的其他设定 / 大纲 / 正文 / 追踪文件
  - 采用 `串行单文件推进`：每个执行步只允许落 `1` 个文件，但当前文件完成后，若用户已明确要求“继续 / 不要停 / 一直往下推”，则必须立即进入下一个执行步，不再重复询问
  - `单文件` 约束的是“单个落盘动作”，不是“单个用户回合”。同一用户回合内可以连续执行多个执行步，但每一步都只能新增或改写 `1` 个文件

- `单文件推进` 默认优先级高于“批量补齐模板”的习惯动作。
- 该规则只约束冷启动测试 / 用户显式要求单文件推进的场景；正常正式项目仍按对应写作 skill 的标准流程执行。

## 回修模式提示

- 当用户意图是“修改第X章 / 回炉 / 重写第X章 / 审后继续改”时，路由到 `story-long-write` 或 `story-review` 前，默认附带以下判定提醒：
  - 先判断当前问题是“单一问题”还是“复合问题”
  - 单一问题才允许“补丁式抠句子”
  - 若同章连续 2 轮仍是同主失败码，或问题同时跨句子层/章节层，或同时牵动确认链、收益落点、人物压场中的 2 项以上，默认按“一次性收口重写”处理
- 路由器只负责提醒，不单独裁决；实际裁决由 `story-long-write` / `story-review` 落盘执行

## 项目状态感知

路由前先检查当前项目状态：

- **无项目目录**（没有包含 `追踪/` 或 `设定/` 的书名目录）：
  - 如果用户要写作，建议先用 `story-setup` 初始化环境
  - 如果用户要扫榜/拆文，直接路由
- **已有项目**：检查 `.story-deployed` 标记，如未部署建议先用 `story-setup`
  - 如果用户要扫榜/拆文，直接路由
- **已有项目**：检查 `.story-deployed` 标记，如未部署建议先用 `story-setup`
