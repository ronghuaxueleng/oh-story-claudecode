...
- 最后有效节点必须进入原文最后 10%
- 任一条件不满足，状态固定为 `blocked-on-source-coverage`，不能生成完成态
...
```text
F01 | L起-L止 | 锚点：原文短语 | 类别：主体边界 | 主体：角色 | 动作：原文动作 | 结果：原文结果 | 口径：原文明确 | 禁止越界：不能改写成什么
```
...
- `写作资产/桥段施工卡.md`：必须把最值钱的 3-5 个桥写成“人类可直接调用”的厚卡，至少每张卡都覆盖：
  ...
  - `为什么这个顺序不能乱`
  - `后续调用方式`
  - 其中至少 1 张必须是中段承重桥，不能只保留开头钩子和终局炸场
...
- `写作资产/profile_source.md` 里如果缺 `公开场硬件 / 外部秩序件 / 后果链 / 感情伤抬升到现实伤的节点 / 秩序回正节点 / 长尾惩罚节点 / 离场 / 换图节点 / 容易写成作者判词的句型 / 容易写成主题总结的句型 / 容易写成整齐揭露的句型` 里的任意关键字段，视为未通过
- `写作资产/桥段施工卡.md` 里如果少于 3 张桥段卡，视为未通过
