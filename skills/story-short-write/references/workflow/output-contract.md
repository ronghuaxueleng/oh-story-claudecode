...
- 必须先由 `写作资产/全文情绪颗粒总账.json` 从 L1 到 EOF 盘清全部实际情绪拍；`profile_source.md / 桥段施工卡.md / 高敏桥段识别.md` 的每个 BID 再引用本桥对应的总账原序子集。每拍带稳定 `beat_id / 实际作用 / 烈度 1-10 / 独占原文证据`。禁止预设角色目录、合并同类重复拍，禁止删掉 `bid_ids=[]` 的桥外拍。
...
- `fake_signals`
- `recommended_sequence`
- `emotion_sequence`：原文逐句盘点出的实际拍全集，不预设拍数；每拍含 `beat_id / role / content / intensity / source_evidence`，证据不得跨拍复用
