...
先写 `设定.md`，再逐拍替换主体 P 拍事件壳并完成 `小节大纲.md`；用户已给 JSON 脑图时，先归一化脑图目标节点，再补足分节施工信息。细纲按导语、连续数字节、尾声顺序，每批 1-2 个相邻区域直接写入同一个正式文件；每个区域必须同时写入入场状态和离场状态，落盘后立即运行 `preflight --allow-partial`。每条细拍首次落盘前，先分别核对 P 的动作/控制/信息/后果，E 的内容/触发/关系位置/读者效果/烈度，以及层型、进出关系、保留规则和六维；整条 E 必须由单一节点完整承接。核对后才在行尾登记隐藏 `source-map`，禁止按相邻 ID 机械分配、把 E 拆给前后节点或先写概述占位。
