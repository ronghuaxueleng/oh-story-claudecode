...
P/E 顺序、事件换芯、主体声线、SF 表演链、来源层型、叙述距离和全部真实句子的通读与修正，均并入当前区域候选审查和 `confirm-section` 所需检查；确认后不另设逐节审计或默认改写。逐节判断只写回正式 `规则执行台账.json`，不创建暂存稿或独立侧车。实际发现上游目标节点不能施工时，定点修改目标骨架并正式增量重绑。

## Phase 4：紧凑终审

全文写完后，按节点审查策略完成完整初稿独立审查，运行 `validate-draft --require-complete` 和默认模式的 `validate_streamlined_write_release.py`；通过后再运行 `audit-init`。默认使用 `audit-confirm-compact`，只提交逐目标节点和逐来源层的正文引句、实现结论与保真布尔；目标脑图在正文前已经锁定 P 四项、E 五字段、SF 表演链、层拓扑和六维，终审不再复制第二套逐字段语义。所有目标节点和来源层都必须显式提交，脚本不得因存在一条引句而自动全置真。高风险项目仍可改用旧的逐字段 `audit-confirm-*` 接口，但不得并行维护两种回执。
