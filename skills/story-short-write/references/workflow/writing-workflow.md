...
先写 `设定.md`，再逐拍替换主体 P 拍事件壳并完成 `小节大纲.md`；用户已给 JSON 脑图时，先归一化脑图目标节点，再补足分节施工信息。细纲按导语、连续数字节、尾声顺序，每批连续 3-5 个区域直接写入同一个正式文件。每条细拍首次落盘前，先分别核对 P 的动作/控制/信息/后果，E 的内容/触发/关系位置/读者效果/烈度，以及层型、进出关系、保留规则和六维；整条 E 必须由单一节点完整承接。核对后才在行尾登记隐藏 `source-map`，禁止按相邻 ID 机械分配、把 E 拆给前后节点或先写概述占位。
...
- 每个来源层逐项填写层型、进入、退出、叙述距离、全部 `must_preserve_in_target` 和六维的具体目标节点实现，并确认无功能顺移。
...
放行闸确认项目配置、profile、目标脑图、P 拍换芯、逐 E 五字段、SF 表演链和逐层写前保真有效后，才写 `正文.md`。每个区域只读取对应目标节点，再按层 ID 回查来源脑图和原文行号；现场、概述、插嘴、跳时、急刹和余尾保持各自层型与连接。跨区域 SF 仍作为一个连续写作单元。

每完成一节就在正文上通读和修正 P/E 顺序、事件换芯、主体声线、SF 表演链、来源层型和叙述距离。不为这些动作创建逐节人工证明。发现上游目标节点不能施工，修改目标骨架后正式增量重绑。

## Phase 4：紧凑终审

全文写完后运行 `audit-init`。`正文覆盖回执.json` 不复制来源静态正文，但为 P 四项承重、E 五字段、目标节点以及层拓扑/每条保留规则/六维分别保存专属正文引句和结论。所有保真布尔必须由当前模型显式提交；脚本不得因存在一条引句而自动全置真。
