...
为了避免后续流程仍然依赖 skill 外部目录，下面这些文件现在已经复制进 `story-short-write` 内部：

- 规则原件目录：`references/myconfig-import/`
- 预检脚本副本：`scripts/precheck_rewrite_gate.py`

当前 skill 内 `myconfig-import/` 包含：
...
- 写作流程、回修流程、第二道闸门，默认优先引用 skill 内副本
- 外部规则仓仍是上游来源，但 skill 运行时不应再默认依赖外部绝对路径
- 如果外部规则后续继续补强，需要同步回灌 skill 内副本
...
- `references/myconfig-import/precheck_rewrite_gate.config.json` 现在只允许承载跨题材底座规则；书级专项补充应单独增设 override 配置，不污染底座
