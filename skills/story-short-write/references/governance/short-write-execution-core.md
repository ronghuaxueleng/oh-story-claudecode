...
20. 大纲完成后先运行 `项目工具箱.py prepare-outline`，由脚本在 `写作资产/模型语义输入.json#outline_semantic_task` 生成当前细纲、全部选中原文和逐节来源绑定的机械任务。当前执行模型必须完整实读全部 `global_source_reads`，再逐节填写 `source_slice_reviews`：`source_range` 必须使用精确 `L起始-L结束`，证据必须位于该行段内，六项 `style_dimension_reviews` 必须分别写出原文观察、原文证据和目标迁移方式。完成这些任务并把完整语义写入 `outline_compilation` 后，才能把任务状态改为 `completed` 并运行 `项目工具箱.py compile-outline`。任务 pending、SHA/指纹失效、来源不一致、切片越界、证据位于切片外或任一文风维度空填时，必须在 Node 编译器启动前阻断。编译通过后再运行 `项目工具箱.py start-draft`，统一复验正文放行并初始化首稿入口与逐节执行回执。旧项目迁移、skill 路径变更或历史回执 SHA 失效时，才使用 `compile-outline --from-existing-receipts`、`--legacy-data-module` 或底层入口的 `--auto-refresh-legacy-bindings`。正文目标文件已有内容、已出现数字小节、缺逐节颗粒包或任一放行回执失效时，入口直接阻断。每节运行 `write-section N`，装载并完整输出该节颗粒包后重读全部 `source_slice_bindings` 和生成契约，只写当前节；正文写完后把停检语义填回同一创作语义源，再运行 `write-section N --phase close`。需要回炉时运行 `rewrite-section N`，未关闭不得进入下一节
21. 项目内便捷入口脚本统一由 `generate_project_tool_wrappers.py` 自动生成，默认生成 Python 包装脚本，不再维护 `sh`。`写作资产/修复旧项目绑定.py`、`写作资产/运行正文放行.py`、`写作资产/初始化首稿入口.py`、`写作资产/校验首稿入口.py`、`写作资产/项目工具箱.py`、`写作资产/项目总诊断.py` 通过 skill 侧 `templates/project_scripts/manifest.json` 显式登记。正常写作主入口固定为 `bootstrap-book / prepare-prewrite / prepare-setting / prepare-outline / compile-outline / start-draft / write-section / rewrite-section / finish-preview`；其余工具箱命令只用于旧项目迁移和定向调试。`项目总诊断.py` 直接输出当前 gate 阻断点与 `keep / rebuild / invalidate` 文件清单。wrapper 生成按脚本用途分别校验前置文件，允许新建项目先落本地工具箱，再随着回执补齐更多包装脚本；`bootstrap_short_project.py` 默认会自动生成这些本地入口。登记后会从 `templates/project_scripts/{项目名}/` 一并回写对应辅助脚本。旧的 `运行正文放行.sh` 只作为待清理遗留物，不再新增
22. **从拆书资料冷启动新书时，颗粒度契约必须先初始化再写书**：凡 `primary-source` 指向本地 `book.profile.json` 且模式为 `direct_imitation`，必须先运行 `项目工具箱.py bootstrap-book`。该步骤至少生成 `写作规则读取回执 / 拆文读取回执 / 设定顺序契约回执 / 顺序契约回执 / 开头承重契约回执_大纲 / 细纲表演验收回执 / 首写容量契约回执 / 模型语义输入 / 冷启动执行清单`；`规则执行台账.json` 必须在两份读取回执通过后由 `prepare-prewrite` 初始化。旧 `cold-start-from-source` 仅保留迁移兼容，并可继续生成 scaffold；新项目不得维护 scaffold 数据文件。在这些前置文件存在并完成相应人工语义前，禁止直接写 `设定.md / 小节大纲.md / 正文.md`；否则视为流程违规，不是普通内容返修
23. 全文落笔后运行 `finish-preview`，由工具箱初始化人工基础审计回执并固定母稿、执行基础审计校验，再标记首稿预览并停靠；仿写稿同步绑定本轮涉及小节的原文切片，只查 `句间关系与虚词 / 段落气口与电报文 / 人物情感过程与动作标签化 / 人物口气与明显剧情断裂`。发现基础硬伤时按母稿与原文双基线回修，再重新运行 `finish-preview`；此时不得继续跑原文基线、窗口前回修、人工分窗、正式审计、最终台账重绑或人工语义复核
24. 只有用户看过首稿并明确确认继续后，才执行兼容入口 `confirm-deep-review`，进入以下深审流程
25. 用 `validate_sequence_contract.py extend-draft` 绑定正文，保留已审核设定/大纲证据；只补正文节点证据并通过 `validate --draft ...`
26. 对正文前 `20 / 60 / 80 / 120` 字再次执行 `opening_contract_gate`
27. 首轮按 skill canonical 规则和主体拆书资产做正文定向回修；仿写稿先固定本轮母稿、重新实读每个待改区块对应原文切片，再逐项留下原文、母稿和改后正文证据
28. 通过 `validate_pre_window_revision_gate.py`
29. 仿写 / 融合 / 同桥任务先建立原文审计基线，再导出人工模型分段任务并逐节点人工切窗
30. 跑正式全量审计、逐窗人工裁决并生成回修任务单
31. 按正式审计施工单回修；仿写稿每个待改区块仍须重新读取对应原文切片并建立本轮双基线，不能沿用上一轮概括。正文 SHA 变化后，对应顺序契约、窗口前回修回执、人工分段回执、正式审计和原文基线对照全部失效
...
- `outline_semantic_task` 是细纲语义生成输入，不是形式回执；必须先由当前模型完成全原文实读和逐节精确切片复核，再允许 `compile-outline`。六项文风颗粒只勾选布尔值、证据只存在于全文但不在绑定切片内，均视为未消费原文颗粒
...
python3 "{项目目录}/写作资产/项目工具箱.py" sync-sources
...
首次运行会从逐节实读记录中归并原文路径，自动初始化 `写作资产/首稿基础审计回执.json` 和母稿，然后停下等待模型人工填写。四项人工检查都必须引用当前正文原句；仿写稿还必须填写原文颗粒基线。发现问题时，在同一回执填写逐区块双基线记录，完成回修后更新最终正文绑定，禁止重新初始化覆盖母稿。填写并通过后再次运行：

```bash
python3 "写作资产/项目工具箱.py" finish-preview
```

复跑会自动生成完成状态的机械绑定、校验并标记 `draft_preview`。命令通过后必须立即向用户交付首稿并停止。只有用户明确确认继续深审后，才记录确认并恢复流程：

```bash
python3 "写作资产/项目工具箱.py" confirm-deep-review \
