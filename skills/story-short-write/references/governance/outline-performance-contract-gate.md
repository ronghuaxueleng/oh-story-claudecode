...
- `hot_news_materials`：真实检索的热点来源、可迁移机制和事实边界。
- `granularity_coverage`：由主体 SF 原文行区间、主体 P 拍和 P 目标映射自动派生，覆盖固定六维文字颗粒。
- `manual_confirmation`：一次确认上层结构与 E 拍完整、P 拍逐拍换芯、主体唯一声线、主体六维全集、热点事实边界和辅助机制边界。
...
- 主体上层 BID/E/SF 层级必须与来源账本完全一致；热点不得修改该层。
- 每个主体 P 拍必须一对一替换，至少改变三个事件壳维度，其中两个是核心现实机制维度。
- 至少两条不同热点新闻落到两个目标 P 拍；只有一个主体 P 拍时最低数量降为一。新闻发布至检索不得超过 90 天。
