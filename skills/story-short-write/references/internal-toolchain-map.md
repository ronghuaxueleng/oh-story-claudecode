...
- `story-deslop/references/myconfig-import/`

当前文件与 `story-short-write/references/myconfig-import/` 对齐：
...
2. 再读 `references/audit-rulebook.json`
3. 再读 `references/myconfig-import/precheck_rewrite_gate.config.json`
4. 再读 `references/myconfig-import/通用高风险词类词典.json`
5. 最后才允许人工参考层文档参与判断
6. 第二闸门回执回填后，还要先过 `validate_gate_receipts.py`
7. 两份回执都过校验后，还要重刷同轮 `cycle_summary.json / gate_validation.md / STATUS.txt`
...
这批规则的来源是 `仿写-幼薇` 的实战复盘，不是整包照搬，而是拆层吸收。
