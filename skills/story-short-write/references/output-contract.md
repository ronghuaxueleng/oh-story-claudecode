...
**文件名约定**：`拆文报告.md / 情节节点.md / 写作手法.md` 由 `story-short-write` 硬编码消费，不能改名。分析叙事走 markdown，数字和枚举走 `_meta.json.structure_counts`。
...
扫描 `拆文报告.md` 全文，对照 `references/banned-words.md` + `references/anti-ai-writing.md`。
...
扫 `story-short-analyze/references/output-templates.md` 里所有 `[BLOCK]` 标注项，看对应产出段是否在 `拆文报告.md` 出现。
...
> `story-short-write` 当前硬编码读 `拆文报告.md / 情节节点.md / 写作手法.md` 这三个 markdown。
> `_meta.json` 是可选增强：read 要容忍，不存在也不能阻塞写作。

| 文件 | 角色 | 怎么读 |
|------|------|--------|
| `_meta.json`（可选）| 数字门面 + 题材识别 | 看 `genre_detected` 决定用哪个题材标尺，读 `structure_counts` 确认拆文完整性，读 `structure_counts.reversal_type` 选反转骨架 |
| `可直接仿写_*.md`（可选但强烈建议） | 仿写/融合直接输入 | 优先按 `导语拆解 → 顺序事件 → 物件 → 动作 → 对白功能 → 对话衔接 → 误判 → 钩子 → 微动作 → 安静压迫场` 顺序读取；缺任意 2 张以上，默认仍停在概括层 |
...
1. 看 `_meta.json.genre_detected` 和 `structure_counts.reversal_type` 先选骨架。
2. 如果目录里有 `可直接仿写_*.md`，先按 `导语拆解 → 顺序事件 → 物件 → 动作 → 对白功能 → 对话衔接 → 误判 → 钩子 → 微动作 → 安静压迫场` 读取。
3. 再读 `拆文报告.md` 里的“核心手法”“共鸣分析”“可复用结构”，决定哪些保留，哪些调整。
4. 读 `情节节点.md`，把节奏锚点抄到新故事的字数位置上。
5. 写场景时翻 `写作手法.md` + `原文/`，参考具体写法。
6. 写完后可选：在新文档 frontmatter 写 `derived_from: 拆文库/{书名}/`，方便追溯。
...
# 通过：输出 8000+ 字同题材新短篇，prose 带着源文的对话节奏和画面感
# 失败：写得像填空，或 short-write 找不到这三个 markdown
```
