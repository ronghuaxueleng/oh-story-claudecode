...
- `run_full_ai_audit.py`
  ...
  - 默认不导出人工模型分段回执；正式审计直接完整读取正文，窗口回执仅作为用户明确要求时的可选诊断
...
5. 回填后必须分别跑：
   - `python3 "$SKILL_ROOT/scripts/validate_gate_receipts.py" "项目目录/写作资产/rewrite_gate_receipt.json" --require-executed --require-complete`
   - `python3 "$SKILL_ROOT/scripts/validate_gate_receipts.py" "项目目录/写作资产/failure_gate_receipt.json" --require-executed --require-complete`
