...
- `story_short_write_project_toolbox.py`
  ...
  - 正常写作主入口收口为 `bootstrap-book / prepare-prewrite / prepare-setting / compile-outline / start-draft / write-section / rewrite-section / finish-preview`；细纲与逐节停检语义只维护在 `模型语义输入.json`，规则分类和模型复核计划仍由 `规则执行台账.json` 管理，其余机械回执由脚本编译。
  - 继续提供 `prepare-outline / prepare-draft / refresh-bindings / validate-outline / validate-opening / init-setting-sequence / validate-setting-sequence / extend-outline-sequence / validate-sequence / extend-draft-sequence / draft-release / sync-sources / init-first-draft / validate-first-draft / init-first-draft-basic-review / validate-first-draft-basic-review / validate-section-execution / open-section / close-section / reset-section / generate-wrappers / cold-start-from-source / init-completion / validate-completion / mark-draft-preview / confirm-deep-review / audit-local-stiffness / audit-project` 作为迁移兼容和定向调试入口。
...
- `initialize_cold_start_from_source_profiles.py`
  - 从本地拆书 profile 冷启动新书的底层实现，由项目工具箱的 `bootstrap-book` 调用
  - 一次性初始化 `写作规则读取回执 / 拆文读取回执 / 设定顺序契约 / 顺序契约 / 开头承重契约_大纲 / 细纲表演验收回执 / 首写容量契约 / 模型语义输入`；规则台账等待两份读取回执通过后由 `prepare-prewrite` 初始化
  - 新项目不生成 scaffold；旧 `cold-start-from-source` 兼容入口仍可生成 scaffold 供迁移使用
