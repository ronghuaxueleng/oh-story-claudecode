...
当调用方开启蒸馏模式时，额外把“颗粒度丢失”视为阻断级风险：如果候选只交代结果或关系结论，没有逐步呈现人物注意力、动作/受力、承载物、信息变化和可见后果，必须引用候选中的连续原文判定 `revise`。不得因为总字数、P/E 标签或事件结果已经出现而放行。对白若复述细纲字段、机制名称、关系结论或作者分析，也必须判为 `dialogue_spokenness`，要求回到眼前事实、物件、名字、钱、门、合同、名单或命令。
