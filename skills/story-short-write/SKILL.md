...
书名必须在主卖点清楚后锁定，不能用“题材 + 具体物件 + 离婚/分手”的公式抢跑。锁名前必须先写清 `题材承诺 / 主卖点 / 核心情绪 / 付费期待`，再生成 `3-6` 个方向不同的候选，并逐个检查：
...
用户未指定书名时，执行器可以推荐首选，但必须先展示候选对比和推荐理由，再锁名、建目录。禁止先建一个事件说明式目录，再让该标题反向绑架设定。

反例：`他让白月光穿走我妈的婚纱后，我离婚了`。问题是语序生硬、信息堆满、提前锁死婚纱物件，且主要承诺停在“我离婚了”，没有形成追妻期待。

正例方向：`他以为我永远不会走`。它用自然口语留下男主误判和失去后的追妻期待，不提前泄露具体桥段。
