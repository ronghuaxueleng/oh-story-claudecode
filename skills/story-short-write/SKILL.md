...
主体 `子流程索引.jsonl` 是必需来源资产。脚本校验每个 `SF-*` 都具备叙述态度、句间节奏、段落气口、对白错位、动作感知情绪织入、叙述者插嘴与毛边六维颗粒，再按原文行区间和主体 P 拍映射自动派生到目标区域；合同同时保存每个 SF 六维的来源分析与证据要求。不得新增人工 SF 映射回执，也不得把辅助来源接入文字颗粒。
...
现有 v4 合同由这条命令或 `batch_outline_release.py` 原地升级为 v5，保留证据未变的映射并确定性补齐六维要求；不得删除合同绕过迁移。
...
全文完成后只建立 `初稿终审回执.json`（当前 schema 为 v3）。每个正文区域一次确认 P 拍换芯已兑现、E 拍完整、场面成形、原事件壳已拒绝、主体声线匹配，并对该区域每个 `SF-*` 的六个维度分别回填 `realized`、当前正文逐字引句和改编说明；缺任一维、使用 `partial`、引句不在当前区域或说明过短都必须阻断 `seal`。外部检测器的任意字数切片只能作为诊断，不能替代按合同 SF 单元的完整六维核对。合同存在热点时，使用热点的区域另引至少一条机制落地句，并在全局检查全部热点机制与事实隐私边界；未启用热点时不填写、不强制这两类判断。其余全局检查完整上层层级、全部主体 P 拍替换、全部主体 SF 覆盖、题面、实际开头、结尾后果、长句换气、对白效率和声线边界。不得把“全局声线像”或一条区域引句冒充 SF 六维全集已消费，也不得另填问题修复清单；发现的问题直接修正文后运行 `refresh-derived`。
...
# init 后修改正文时运行；旧 v2 回执也用此命令升到 v3，但不会继承旧的区域级颗粒判断
