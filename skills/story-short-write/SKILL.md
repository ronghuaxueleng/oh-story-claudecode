...
审查调度以 [节点审查与首写预检](references/governance/checkpoint-review-policy.md) 为准，执行前必须阅读。用户要求减少审查调用时启用 `whole_book`：区域只运行结构、顺序、哈希、颗粒和格式门禁，不做逐节 critic、盲审或逐句复核；全部细纲完成后统一审查一次，全部初稿完成后统一审查一次。全颗粒度指覆盖粒度，不等于外部调用频率。

整纲与整稿通过必须包含该政策的“对照证据合同”：直接核对设定、跨区状态、逐拍功能、来源层边界与结局。SHA、编号齐全、preflight 或四轴总评不能代替内容审查。续跑发现旧 pass 与正式文本矛盾时，按实际缺口补审，不继承错误结论；不因此重开项目或重读全部来源。
一次只写入一个正式区域；整纲审查检查对白人话风险，但不提前代写正文措辞。
## 连续执行终止硬闸
...
用户已授权时，当前执行暴露的规则冲突、脚本缺陷与重复性写入应在原正式入口定点修复，随后续跑；不另建维护阶段或回执。
...
新增场面资产边界：人物类型、职业现场、场域、物件和现实操作不得由模型凭空补壳。形成设定或细纲候选前，先从本轮允许读取的全部拆文结果中按题材、现实权限和当前 P/E 功能筛选可迁移资产；每项候选记录来源书、原资产类型、适用拍位和去人物、去原句、去完整事件链的改编决定。主体拆文仍独占声线与表演组织，辅助拆文可提供候选场面资产但不提供声线、对白、E 拍或人物关系答案。未被筛选绑定的资产不得进入正文；筛选资产也必须重新落到本书设定的人物、职业权限、空间和物件，不得把原作专名或整套场景换名复刻。
...
顺序完成 `设定.md` 和 `小节大纲.md`。两个阶段都先在当前工作上下文形成**未落盘候选**，交给独立 diagnostic-only critic；critic 不接收写作者辩护，只看候选、当前规则 case、项目配置和实际来源资产。至少审查一个初稿 weakest link；存在真实 finding 时 writer 只定点修复，不得为满足修复次数捏造问题。无实质错误时可提交空 `draft_findings`，但必须以 `weakest_link_review` 的候选逐字引句、真实规则引用、具体风险和无需改写的理由证明已审查最弱处；全部阶段审查轴仍须清零，才运行 `precommit-design`。候选和初稿不写临时文件；台账只保存最终候选 SHA、动态失败码、逐字证据和裁决，不保存第二份设定或大纲。
...
设定完成后逐拍重建目标 P 拍；`whole_book` 模式允许按一条完整表演链批量形成多个连续数字节候选，仍按顺序逐区落盘并运行确定性预检，不重复读取整张来源脑图，也不做逐节 critic；全部区域完成后统一检查入场/离场状态、P/E 整拍、来源层型、信息取得、物理受力、现实操作、未来对白人话风险和未来区域泄漏。
...
```bash
python3 "$SKILL_ROOT/scripts/validate_rule_execution_ledger.py" reopen-setting \
  --ledger "{项目目录}/写作资产/规则执行台账.json" \
  --user-authorization "{用户明确授权原文}" \
  --reason "{改变范围与必要性}"
```
...
区域级字段的独立性不得靠编号、括号尾缀、情绪标签或“本区/下一区”替换伪造。生成候选后，必须先删除区域编号、括号说明和流程套话，再对 `主事件 / 子事件 / 情绪 / 读者新获知什么 / 钩子 / 伏笔/物件 / 动静 / 对话密度 / 场面单元` 做语义去重；任一字段与其他区域高度相似，必须回到对应 P/E/SF/L 重新施工。细拍正文中的主体人物姓名、核心关系壳和专属物件也必须全部换壳；隐藏 `source-map` 只能登记 ID，不能把主体专名带回目标证据。官方 `preflight` 执行近重复、来源绑定与顺序校验；专名及人物壳复用由模型对照原文在现有自检和整纲审查中裁决，不得从 `actor` 按字数猜姓名或维护职业例外表。任一检查失败不得确认区域。

允许在当前区域内一次形成、批量传递和编译多条细拍；“逐拍核对”要求每拍都有独立内容与对应依据，不要求每拍单独调用模型或脚本。禁止的是批量制造重复、模板话、概述话细拍，不是批量处理本身。模型必须依据当前来源行域、上一节离场状态和设定，为各拍分别确定动作、承载物、信息变化与后果；仅换编号、人名或物件名的固定句壳，以及用剧情标签替代现场链，均不合格。原文自身的概述、跳时或结果层仍保留原有叙述距离，不因此被判为概述话。

正式脚本可批量接收模型已经逐拍形成的具体细拍，执行结构编排、显式来源绑定、哈希、顺序校验与反模板检测；输入必须包含每拍由模型写出的独立动作、承载物、信息变化和后果。脚本不得硬编码或臆造场景、物件、情绪、对白、信息变化、钩子或后果，不得用编号、相邻拍位、词库或固定句壳自动补齐缺失语义。每个区域仍按 critic、precommit、preflight、confirm 顺序审查和落盘，不提前生成未来区域。批量候选出现缺陷时，在当前区域定点修复；只有实际采用“先用模板批量铺满正式文件，再靠预检修正”的违规流程才按流程污染处理，不能仅凭一次提交多条细拍要求重开项目。拍数、层数多或需要分批执行都不是外部阻断或终止理由；重复性结构处理应并入正式脚本，不另造临时写入脚本。
...
下一区域只读取其所需的主体原文行域、来源层、上一整区域离场状态和当前设定；批量施工时也只读取当前表演链行域，不得为方便生成而一次性读取并重写全书。每条 `细拍拆分` 首次写入候选时必须在行尾写隐藏的来源覆盖注释；大纲 critic 的 `source_refs_considered` 必须与当前候选全部声明逐项同序一致。注释只登记 ID，不重抄来源内容：

区域级施工字段（`主事件`、`情绪`、`钩子`、`伏笔/物件`、`场面单元`）必须针对当前区域独立填写；不得把同一组通用模板复制到多个区域。场面单元中的人物、职业、场域、物件和现实动作必须能回指已筛选的拆文场面资产或设定事实，不能用“适合这里”“增强冲突”等空泛理由。正式 `preflight` 会对这些字段做完全重复拦截；命中后必须回到当前正式文件逐区重写，不能用改标题、改数字或保留重复字段继续下游。
...
数字节数量与目标字数是细纲阶段的硬合同，不得等正文放行才发现。细纲完整预检后必须立即运行 `validate_streamlined_write_release.py --project-dir "{项目目录}" --outline-only`；此模式只验已逐区冻结的完整细纲、来源绑定和篇幅，不要求尚未生成的目标脑图，也不放行正文。若主体数字节数下限未满足、整体/分节上限超出主体锚定比例，必须在细纲阶段重切区域并重新逐区确认，禁止靠正文阶段压缩或补字绕过。正文前仍须不带 `--outline-only` 运行默认放行，已存在正文的项目不得使用细纲模式。
...
通过后只处理当前区域：首行用 `# 《书名》`，随后写导语，再按 `1.`、`2.` 直到 `N.` 的知乎纯数字分节顺序逐区推进，尾声并入最后一个数字节。当前区域先在本轮工作上下文形成候选，不得写入 `正文.md`、不得落临时文件；候选通过确定性颗粒、长度和顺序门禁后才允许第一次追加到正文；`whole_book` 模式不执行逐节盲审。正文文件首次只能落盘标题和已 precommit 的导语；当前区域的确定性门禁通过后，才允许进入下一个区域。不创建暂存稿、分节草稿或独立逐节侧车，候选哈希、盲审与正式复核只保存在 `规则执行台账.json`。

写每个区域前必须先运行 `prepare-section`。首次领取或规则 SHA 变化时，脚本完整输出已确认规则组及 cases；规则未变化的后续区域只输出规则组摘要和当前增量 cases，并带 `rules_context_sha256`。当前区域绑定的目标节点、P/E、全部来源层完整原文行域、连续来源句链、节奏、气口、主体 `sentence_motion`、上一节尾句和颗粒合同仍完整输出。正文模型必须在当前区域写作上下文中逐组消费这些规则；不得只依据细纲、句法包摘要或记忆中的规则写作。输出直接作为当前区域写作上下文，不落新文件。未领取写前句法包时禁止写当前区域；正文已经提前出现当前区域或未来区域时，命令必须阻断。
...
`plan-section` 是首写施工门，不是形式回执。每个来源句必须在计划中拥有独立的目标承接句设计，并明确动作/受力、物件、人物注意力、对白或静默、结果与断口；每个目标节点还要有自己的连续动作链和关系后果。计划中出现“概括、总结、随后推进、完成控制变化、获得或失去一项”等流程词，或只写“写出动作/保留现场”而没有本区对象与落点时，脚本必须阻断。只有计划通过后，模型才允许形成正文候选；首写不得先交一版剧情提要再靠盲审补颗粒。计划中的每个目标承接句还必须预先标明断句边界（sentence_breaks）、该句唯一动作或信息中心（sentence_focus）以及与上一句的可见承接（sentence_bridge）。同一计划项不得把两个以上独立动作、视线换主或信息释放塞进一个句子；缺少这些逐句字段时必须阻断候选生成。
...
候选完成后切换到独立 critic 任务，不再沿用写作者的解释意图。critic 只接收最终候选、当前句法包的可核验规则引用和项目已记录用户反馈，先作诊断、不直接改写：逐句引用最可疑短语，检查朗读、物理动作、人物注意力和结构化记录；按连续动作链、话轮组与段落转接审查 weakest link。真实 finding 必须定点修复；无实质错误时按设计阶段同一口径提交有逐字证据的 `weakest_link_review`，不得为凑修复次数造错或润色。最终候选的失败码必须清零，逐句、逐组与颗粒覆盖检查均不得省略。失败类型由 critic 针对当前问题动态命名，不使用固定词表；每条 finding 必须引用当前 `规则执行台账.json.groups[].cases` 的真实 `rule_id:line`，脚本只校验引用和结构，不替模型判语义。
...
`per_region`/`checkpoint` 模式下当前区域落盘后，模型必须完整通读该区域全部真实句子并逐句复核；`whole_book` 模式不执行逐节句子复核；与前文或本区重复的句子必须逐条裁决。`split / revise` 尚未改回正文、旧区域 SHA 变化、一次追加两个区域、漏句、漏对白或批量套用相同判断时，`confirm-section` 必须阻断。通过前不得调用下一次 `prepare-section`：
