...
审查调度以 [节点审查与首写预检](references/governance/checkpoint-review-policy.md) 为准，执行前必须阅读。用户要求减少逐节审查时启用 `checkpoint`；下文的区域 critic/盲审在该模式下指如实标记的作者自检，不要求逐节子代理，设定及完整细纲、完整初稿仍独立审查。全颗粒度指覆盖粒度，不等于外部调用频率。
## 连续执行终止硬闸
