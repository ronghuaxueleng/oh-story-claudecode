...
- 检测 `.codex/agents/chapter-extractor.md` 是否存在（Phase 2 长篇深度分析的并行子代理）。

**未部署时**，提示用户：

> 「检测到当前项目尚未部署写作基础设施。建议先运行 `story-setup` 再回来导入，否则深度分析阶段无法使用并行 chapter-extractor 子代理。」
...
| 篇幅 | 调用的拆解管线 | 产物目录 |
|------|--------------|---------|
| 长篇 | story-long-analyze 的 7 阶段管线（Stage 0-6） | `拆文库/{书名}/` |
...
story-long-analyze 在 Stage 0+1（黄金三章）后会**自动停靠**并询问是否继续全量拆解（对应 story-long-analyze 的「Stage 1 停靠点」）。但导入场景需要 Stage 2-6 的全套产物（逐章摘要 / 聚合分析 / 设定关系 / 汇总报告 / 文风），缺一不可——否则 Phase 3 迁移会拿到半成品。
...
- 环境检测（Phase 1）发现未部署 chapter-extractor 子代理且用户选择「继续导入」时，Stage 2 逐章摘要降级为串行处理，产物仍完整，仅速度变慢。
...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色）。每章3-40情节点（密度150-200字/个，按字数动态调节）。角色过滤（龙套不提取、别名归类）。**并行 chapter-extractor 子代理模式**（未部署子代理时降级串行）。**计数验证：摘要数 == 章节数**。 | 所有章节处理完成 |
...
| 6 | 文风 | 拆文报告.md + 黄金三章深度拆解 + 章节摘要 + 原文 | 文风.md（整书级写作技法视图 + 原文锚点 few-shot） | 文风落盘完成 |
...
沿用 story-long-analyze 的分块策略（Stage 2 使用 chapter-extractor 子代理并行，其他阶段按以下策略分块）：
...
将拆文库中的高复用分析文件同步到项目 `参考资料/`：

- `拆文库/{书名}/拆文报告.md` → `参考资料/拆文报告.md`
- `拆文库/{书名}/快速预览.md` → `参考资料/快速预览.md`
- `拆文库/{书名}/文风.md` → `参考资料/文风.md`

全部保留 `[导入反推]` 或拆文来源标记，不改写原分析结论。这样后续 `story-long-write` 在日更时可以直接把 `文风.md` 当参考底稿读取。
...
- 可选验证：如果项目已部署 `story-explorer`（检查 `.codex/agents/story-explorer.md` 是否存在），可 spawn 一个子代理，并按以下信息构造输入：
  - 目标：`story-explorer`
  - 项目目录：`{dir}`
  - 查询类型：`progress`
  - 查询参数：`导入验证`
