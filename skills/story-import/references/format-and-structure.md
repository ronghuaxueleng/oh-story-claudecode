...
<<<<<<< HEAD
- 每小节 800-1500 字（爽文可压缩至 500-800 字/节，按当前题材公式与平台节奏要求调整）
=======
...
3. **一条读者新获知的信息**
<<<<<<< HEAD
4. **3-5 轮对话交锋**（揭示关系/升级冲突，参考通用的对话权力博弈写法。特定场景如独自发现/翻阅日记可标零，按当前 skill 的细纲/设计任务要求执行）
5. **每个子事件三维度织入**：发生+感知+反应织入同一段正文（按通用场景写法处理）
=======
