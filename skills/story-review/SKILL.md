...
- `story-review` 或 `story-review full` → spawn 全部 4 个子代理；如果当前已经在子代理内，自动降级为 solo
- `story-review lean` → 只 spawn story-architect + consistency-checker；如果当前已经在子代理内，自动降级为 solo
- `story-review solo` → 不 spawn 子代理，自身做基础检查
- 未指定 → 默认 full，并告知用户
