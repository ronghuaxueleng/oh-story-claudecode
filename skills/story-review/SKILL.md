...
- `/story-review`、`story-review` 或追加 `full` → 优先 spawn 全部 4 个子代理；如果当前已经在子代理内，核心子代理未部署/异常，或 spawn 失败，自动降级为 solo。
- `/story-review lean`、`story-review lean` → 优先 spawn `story-architect` + `consistency-checker`；如果当前已经在子代理内，任一所需子代理未部署/异常，或 spawn 失败，自动降级为 solo。
- `/story-review solo`、`story-review solo` → 不 spawn 子代理，由当前会话执行基础审查。
...
2. **确认是否允许 spawn**：如果当前已经在子代理内执行，不再递归 spawn，直接降级为 `solo`。
3. **检查核心子代理部署状态**（只检查项目内 agents，不要假设一定存在）：
   - full 必需：`.codex/agents/story-architect.md`、`.codex/agents/character-designer.md`、`.codex/agents/narrative-writer.md`、`.codex/agents/consistency-checker.md`
   - lean 必需：`.codex/agents/story-architect.md`、`.codex/agents/consistency-checker.md`
   - 对每个必需子代理文件，读取 frontmatter，确认 `name:` 与 subagent_type 完全一致；frontmatter 缺失、不可解析或 name 不匹配时视为 malformed agent。
   - 如果 `.story-deployed` 存在且 `agents_version` 缺失或小于 `8`，视为 stale deployment；不要 spawn，降级 `solo`，建议用户重新运行 `story-setup`。
   - 如果目标模式所需任一文件缺失或 malformed，**不要尝试 spawn 缺失/异常子代理**；自动降级为 `solo`，并在报告开头写明：`Fallback: missing agents -> solo` 或 `Fallback: malformed agents -> solo`，列出问题文件，建议用户运行 `story-setup`。
4. **确认子代理工具可用**：如果当前环境没有可用的子代理调用能力，直接降级为 `solo`，报告 `Fallback: agent tool unavailable -> solo`。
5. **运行时失败降级**：如果任何子代理 spawn 返回失败、`subagent_type` 不可用、frontmatter 运行时解析失败或子代理无法启动，停止继续 spawn，改用 `solo` 重新审查，并报告 `Fallback: spawn failed -> solo` 与失败的 `subagent_type`；不要把部分成功的子代理结果当成 full/lean 结论。
...
full/lean 模式下，主会话必须把“审查基准包摘要”直接写进每个子代理 prompt。**不要要求子代理必须读取 `story-review/references/*` 才能完成任务**；子代理可读取 `story-setup/references/agent-references/*` 作为补充，但最终必须遵守本 skill 注入的 rubric 摘要和统一 Findings Schema。
...
5. **形成审查基准包摘要**：把已加载的文件内容或内置 fallback 摘要压缩为 5-12 条审查标准，后续 solo 和子代理都必须使用这份摘要。

**Phase 1.5：可选 story-explorer 预查询**。仅当 `Effective Mode` 仍为 `full`/`lean`、当前允许 spawn 且子代理工具可用时，才可检查 `.codex/agents/story-explorer.md` 并 spawn `story-explorer` 预查设定摘要；`solo` 或子代理递归保护场景下不得 spawn，只能直接 Read/Grep。Prompt 示例：
...
## Phase 2：并行 Spawn 子代理（full/lean 模式）

使用子代理工具并行调用。每个子代理不继承父对话上下文，prompt 必须自包含项目路径、审查范围、文件路径、必要摘录、审查基准包摘要、Rubric Source 和统一 Findings Schema。

**调用规则**：执行 Phase 0 后，只有实际模式仍是 full/lean 时才 spawn。不要 spawn 缺失子代理。

**子代理 1: story-architect**（subagent_type: story-architect）
- full/lean 均调用。
...
**子代理 2: character-designer**（subagent_type: character-designer）
- full 模式调用。
...
**子代理 3: narrative-writer**（subagent_type: narrative-writer）
- full 模式调用。
...
**子代理 4: consistency-checker**（subagent_type: consistency-checker）
- full/lean 均调用。
...
3. **可选事实核查**：如果审查内容涉及需要验证的外部事实（历史年代、地理方位、职业细节等），只有在 `Effective Mode` 仍为 `full`/`lean`、当前不是子代理、子代理工具可用且 `.codex/agents/story-researcher.md` 已部署时，才可额外 spawn `story-researcher` 搜索验证；`solo`、missing/malformed/stale/spawn failed 降级或子代理递归保护场景下不得 spawn，只能在报告中标记“需人工事实核查”。
...
不 spawn 子代理。先按 Phase 1 第 4 步识别目标平台并加载对应 rubric；即使是 solo，也必须用平台 rubric、`story-review/references/quality-rubric.md` 或内置审查基准包校准判断。
